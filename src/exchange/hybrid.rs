...
use crate::exchange::curve::{PrivateKey, PublicKey};
...
impl HpkeCrypto for CryptoProvider {
    ...
    fn aead_seal(
        ...
        let mut enc = crate::symetric::aes_gcm::Aes256GcmEncryption::new(key, nonce, aad)
            .map_err(|_| HpkeError::AeadInvalidNonce)?;
        let mut output =
            Vec::with_capacity(msg.len() + crate::symetric::aes_gcm::Aes256GcmEncryption::TAG_SIZE);
    ...
    fn aead_open(
        ...
        let mut dec = crate::symetric::aes_gcm::Aes256GcmDecryption::new(key, nonce, aad)
            .map_err(|_| HpkeError::AeadInvalidNonce)?;
        let (msg, tag) = msg
            .split_last_chunk::<{ crate::symetric::aes_gcm::Aes256GcmDecryption::TAG_SIZE }>()
